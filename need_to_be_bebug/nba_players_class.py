...
class SCRAPPING(object):
    ...
    def call_session(self,call_who='localhost'):
        ...
        conn = create_engine('mysql+pymysql://root:root@/cloudsql/brian-db-233409:us-central1:brian-new-sql:3306/nba_cloud',poolclass=NullPool)
        #set long timeout
...
def find_pic(name):
    ...
    print(last_name[0])
    url_head = 'https://www.basketball-reference.com/players/'
    url_first = last_name[0] + '/'
    url_body1 = last_name[:5]
    url_body2 = first_name[:2]
    url_tail = '01.html'
    url = url_head + url_first + url_body1 + url_body2 + url_tail
    try:
        main_text = TEXT(url).get_page_text()
        jpg = main_text.rpartition('<img itemscope="image" src="')[2].rpartition('" alt="Photo')[0]
    except:
        sys.exit(0)
    return jpg
